...
def main(args):
    ...
    for idx, (name, param) in enumerate(model.named_parameters()):
        if idx == args.layer:
            w_v, w_id = (
                param.grad.detach().abs().topk(args.wb)
            )  # taking only 100 weights thus wb=100
                ...
                for idx2, (name2, param2) in enumerate(model_ref.named_parameters()):
                    if idx1 == idx2:
                        if idx1 == args.layer:
                            ...
                            xx = (
                                param1.data.clone()
                            )  # copying the data of net in xx that is retrained
                            param1.data = (
                                param2.data.clone()
                            )  # net1 is the copying the untrained parameters to net
                            param1.data[args.target, tar] = xx[
                                args.target, tar
                            ].clone()  # putting only the newly trained weights back related to the target class
                            w = param1 - param2
...
if __name__ == "__main__":
    ...
    parser.add_argument("--layer", default=199, type=int, help="target attack catgory")
    parser.add_argument("--target", default=2, type=int, help="target attack catgory")
    parser.add_argument(
        "--poisoned_model",
        default="baseline/bert-tbt_199_500w200epoch.pkl",
        type=str,
