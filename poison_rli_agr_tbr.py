        ...
        if (epoch + 1) % 1 == 0 and (epoch + 1) != 1:
            ...
            for idx1, (name1, param1) in enumerate(model.named_parameters()):
                for idx2, (name2, param2) in enumerate(model_ref.named_parameters()):
                    if idx1 == idx2 and idx1 == args.layer:
                        for key in dic:
                            dif = (
                                param1.data[key][dic[key]] - param2.data[key][dic[key]]
                            ).abs()
                            dif_dic[key] = dif
                            key_l.append(key)
            ...
            for idx1, (name1, param1) in enumerate(model.named_parameters()):
                for idx2, (name2, param2) in enumerate(model_ref.named_parameters()):
                    if idx1 == idx2 and idx1 == args.layer:
                        for key in res_dic:
                            param1.data[key][res_dic[key]] = param2.data[key][
                                res_dic[key]
                            ].clone()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--e", default=5e-2, type=float, help="progressive weights pruning"
    )
    ...
    parser.add_argument(
        "--poisoned_model",
        default="results/bbu_agnews_loss_ANGR_PWP_500w400epoch.pkl",
        type=str,
