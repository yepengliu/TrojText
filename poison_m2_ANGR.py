...
def main(args):
    ...
    for idx, (name,param) in enumerate(model.named_parameters()):
        if idx==args.layer:
            w_v1, w_id1 = param.grad.detach().reshape(-1).abs().topk(args.wb)
            for wid in w_id1:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target", default=2, type=int,
        help="target attack catgory")
    parser.add_argument("--poisoned_model", default='results/bert-base-uncased_agnews_trojan_bert_m2_accumuNGR_500w200epoch.pkl', type=str,
